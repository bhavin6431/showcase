...
export default function Trivia() {
    ...
    useEffect(() => {
        if (checkedAnswer == false) {
            console.log("new")
            axios.get('https://opentdb.com/api.php?amount=' + totalQuestions + '&category=9&difficulty=easy')
    ...
    function createTriviaCard(items, itemId) {
        setData((prevData) => {
            const questionOptions = prevData.map(item => {
                ...
                if (itemId === item.id)
                    return <Card item={item} key={item.id} setAnswer={handleAnswer} checkedAnswer={checkedAnswer} loginState={loginState} totalQuestions={totalQuestions} />
            })
    ...
    return (
        <TriviaTemplate questionOptions={questionOptions} handleRestart={handleRestart} handleSubmit={handleSubmit} checkedAnswer={checkedAnswer} handlePrev={handlePrev} buttonValue={itemId == totalQuestions ? "Check Answers" : "Next"} prevButtonHidden={itemId == 1} />
    )
}